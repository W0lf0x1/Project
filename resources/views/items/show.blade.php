    ...
    <div class="row">
        <div class="col-12 mb-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('items.index') }}" class="text-decoration-none">Предметы</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('categories.show', $item->category) }}" class="text-decoration-none">{{ $item->category->name }}</a></li>
                            ...
                            <div class="carousel-inner">
                                @foreach($itemImages as $index => $image)
                                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                        <img src="{{ asset('public/storage/' . $image) }}" class="d-block w-100" alt="{{ $item->title }}" style="height: 400px; object-fit: cover;">
                                    </div>
                                @endforeach
                            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <p class="text-muted mb-0">
                                <i class="fas fa-info-circle me-2"></i>{{ $item->condition }}
                            </p>
                        </div>
                    </div>

                    <div class="d-flex align-items-center mb-4">
                        <span class="badge {{ $item->is_available ? 'bg-success' : 'bg-danger' }} me-2">
                            {{ $item->is_available ? 'Доступен для обмена' : 'Недоступен для обмена' }}
                        </span>
                    </div>

                    @if(auth()->check() && auth()->id() !== $item->user_id && $item->is_available)
                        <div class="d-grid">
                            <a href="{{ route('exchanges.create') }}?item={{ $item->id }}" class="btn btn-primary btn-lg">
    ...
    <div class="row mt-5">
        <div class="col-12">
            ...
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-4">
                @foreach($item->category->items()->where('id', '!=', $item->id)->take(4)->get() as $similarItem)
                    <div class="col">
                        <div class="card h-100 border-0">
                            <div class="position-relative">
                                ...
                                @endphp
                                @if($similarImages && is_array($similarImages) && count($similarImages) > 0)
                                    <img src="{{ asset('public/storage/' . $similarImages[0]) }}" alt="{{ $similarItem->title }}" class="card-img-top" style="height: 200px; object-fit: cover;">
                                @else
                                    <div class="bg-light d-flex align-items-center justify-content-center" style="height: 200px;">
                                        <i class="fas fa-image fa-3x text-muted"></i>
                                    </div>
                                @endif
                                <span class="position-absolute top-0 end-0 m-2 badge bg-success">{{ $similarItem->condition }}</span>
                            </div>
                            <div class="card-body">
                                <h5 class="card-title">{{ $similarItem->title }}</h5>
                                <div class="d-flex justify-content-between align-items-center mt-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="ms-2 text-muted small">{{ $similarItem->user->name }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="card-footer bg-white border-top-0">
                                <a href="{{ route('items.show', $similarItem) }}" class="btn btn-outline-primary w-100">
